<template>
  <div class="app-container">
    <div class="filter-container" style="display: flex;justify-content: space-between">
      ...
      <div>
        <el-button type="primary" @click="handleAddMarketMarqueeInfo">新增</el-button>
      </div>
    </div>
    ...
    <el-table :data="list" v-loading="listLoading" border fit highlight-current-row
      ...
      <el-table-column align="center" label="操作"  width="" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button type="success" size="mini" @click="handleEditMarketMarqueeInfo(scope.row)">编辑</el-button>
          <el-button type="danger" size="mini" @click="handleDeleteMarketMarqueeInfo(scope.row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog
      ...
      width="50%">
      <el-form ref="marketMarqueeInfoForm" :model="marketMarqueeInfoParam" :rules="marketMarqueeInfoRules" status-icon label-width="80px">
        <!-- <el-form-item label="封面图片" prop="coverPic">
          <el-upload
            ...
            <img v-if="marketMarqueeInfoParam.coverPic" :src="marketMarqueeInfoParam.coverPic" class="avatar">
            <i v-else class="el-icon-plus avatar-uploader-icon"></i>
          </el-upload>
        </el-form-item> -->
        <el-form-item label="内容" prop="content">
          <tinymce v-model="marketMarqueeInfoParam.content" :id="tid_content" ref="tinymce_content"/>
        </el-form-item>
        <el-form-item label="已读数量" prop="alreadyRead">
           <el-input type="number" v-model="marketMarqueeInfoParam.alreadyRead"><i slot="prefix" class="el-input__icon el-icon-goods"></i></el-input>
        </el-form-item>
        <el-form-item label="点赞数量" prop="thumbsUp">
           <el-input type="number" v-model="marketMarqueeInfoParam.thumbsUp"><i slot="prefix" class="el-input__icon el-icon-goods"></i></el-input>
        </el-form-item>
        <el-form-item label="评论数量" prop="comment">
           <el-input type="number" v-model="marketMarqueeInfoParam.comment"><i slot="prefix" class="el-input__icon el-icon-goods"></i></el-input>
        </el-form-item>
        <el-form-item label="排序" prop="sorts">
           <el-input type="number" v-model="marketMarqueeInfoParam.sorts"><i slot="prefix" class="el-input__icon el-icon-goods"></i></el-input>
        </el-form-item>
        <!-- <el-form-item label="APP主键" prop="appId">
           <el-input v-model="marketMarqueeInfoParam.appId"></el-input>
        </el-form-item> -->
        <el-form-item label="名称" prop="name">
           <el-input v-model="marketMarqueeInfoParam.name"></el-input>
        </el-form-item>
        <el-form-item label="状态" prop="status">
          <el-select class="filter-item" v-model="marketMarqueeInfoParam.status" placeholder="请选择">
            <el-option v-for="item in  statusOptions" :key="item.key" :label="item.label" :value="item.key">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="删除" prop="isDel">
          <el-select class="filter-item" v-model="marketMarqueeInfoParam.isDel" placeholder="请选择">
            <el-option v-for="item in  isDelOptions" :key="item.key" :label="item.label" :value="item.key">
            </el-option>
          </el-select>
        </el-form-item>
        <!-- <el-form-item label="创建人id" prop="createId">
           <el-input v-model="marketMarqueeInfoParam.createId"></el-input>
        </el-form-item> -->
...
<script>
  import { pageListMarketMarqueeInfo, listMarketMarqueeInfo, detailMarketMarqueeInfo, saveMarketMarqueeInfo, updateMarketMarqueeInfo, delMarketMarqueeInfo } from '@/api/MarketMarqueeInfo.js'
  ...
  export default {
    ...
    data() {
      return {
       ...
       statusOptions: [{ label: '审核中', key: 0 }, { label: '通过', key: 1 }, { label: '拒绝', key: 2 }],
        ...
        marketMarqueeInfoParam: Object.assign({}, defaultParam),
    ...
    created() {
      this.pageListMarketMarqueeInfo()
    },
    methods: {
      ...
      picUploadcoverPic(file) {
        getTempAuthToken().then(res => {
          ...
          axios.post(oss.fileUploadUrl, ossData, {
            ...
            .then(res => {
              this.marketMarqueeInfoParam.coverPic = oss.fileUploadUrl + '/' + oss.key
            })
            .catch(error => {
      ...
      // 加载数据
      pageListMarketMarqueeInfo() {
        this.listLoading = true
        pageListMarketMarqueeInfo(this.listQuery).then(response => {
      ...
      // 显示添加弹窗
      handleAddMarketMarqueeInfo() {
        this.dialogStatus = 'create'
        this.dialogVisible = true
        this.marketMarqueeInfoParam = Object.assign({}, defaultParam)
        this.$nextTick(() => {
          this.$refs.tinymce_content.setContent('')
        })
      },
      // 显示编辑弹窗
      handleEditMarketMarqueeInfo(item) {
        this.dialogStatus = 'update'
        this.marketMarqueeInfoParam = item
        this.dialogVisible = true
        this.$nextTick(() => {
      ...
      // 保存或更新
      handleSaveOrUpdate() {
        this.$refs.marketMarqueeInfoForm.validate((valid) => {
        	if (valid){
        		// 主键为空 -> 新增
		        if (this.marketMarqueeInfoParam.id == null) {
		          saveMarketMarqueeInfo(this.marketMarqueeInfoParam).then(response => {
		          ...
		          // 有主键 -> 更新
		          updateMarketMarqueeInfo(this.marketMarqueeInfoParam).then(response => {
		            if (response.code === 600) {
		              this.dialogVisible = false
		              this.pageListMarketMarqueeInfo()
		            } else {
      ...
      // 删除
      handleDeleteMarketMarqueeInfo(item) {
        this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          delMarketMarqueeInfo({ id: item.id }).then(response => {
            if (response.code === 600) {
              this.$message({
                type: 'success',
                message: '删除成功!'
