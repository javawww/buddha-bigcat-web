<template>
  <div class="app-container">
    <div class="filter-container" style="display: flex;justify-content: space-between">
      ...
      <div>
        <el-button type="primary" @click="handleAddMarketDynamicInfo">新增</el-button>
      </div>
    </div>
    ...
    <el-table :data="list" v-loading="listLoading" border fit highlight-current-row
      ...
      <el-table-column align="center" label="操作"  width="" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button type="success" size="mini" @click="handleEditMarketDynamicInfo(scope.row)">编辑</el-button>
          <el-button type="danger" size="mini" @click="handleDeleteMarketDynamicInfo(scope.row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog
      ...
      width="50%">
      <el-form ref="marketDynamicInfoForm" :model="marketDynamicInfoParam" :rules="marketDynamicInfoRules" status-icon label-width="80px">
        <el-form-item label="封面图片" prop="coverPic">
          <el-upload
            ...
            <img v-if="marketDynamicInfoParam.coverPic" :src="marketDynamicInfoParam.coverPic" class="avatar">
            <i v-else class="el-icon-plus avatar-uploader-icon"></i>
          </el-upload>
        </el-form-item> 
        <el-form-item label="内容" prop="content">
          <tinymce v-model="marketDynamicInfoParam.content" :id="tid_content" ref="tinymce_content"/>
        </el-form-item>
        <el-form-item label="已读数量" prop="alreadyRead">
           <el-input type="number" v-model="marketDynamicInfoParam.alreadyRead"><i slot="prefix" class="el-input__icon el-icon-goods"></i></el-input>
        </el-form-item>
        <el-form-item label="点赞数量" prop="thumbsUp">
           <el-input type="number" v-model="marketDynamicInfoParam.thumbsUp"><i slot="prefix" class="el-input__icon el-icon-goods"></i></el-input>
        </el-form-item>
        <el-form-item label="评论数量" prop="comment">
           <el-input type="number" v-model="marketDynamicInfoParam.comment"><i slot="prefix" class="el-input__icon el-icon-goods"></i></el-input>
        </el-form-item>
        <el-form-item label="排序" prop="sorts">
           <el-input type="number" v-model="marketDynamicInfoParam.sorts"><i slot="prefix" class="el-input__icon el-icon-goods"></i></el-input>
        </el-form-item>
        <el-form-item label="名称" prop="name">
           <el-input v-model="marketDynamicInfoParam.name"></el-input>
        </el-form-item>
        <el-form-item label="APP主键" prop="appId">
           <el-input v-model="marketDynamicInfoParam.appId"></el-input>
        </el-form-item>
        <el-form-item label="状态" prop="status">
          <el-select class="filter-item" v-model="marketDynamicInfoParam.status" placeholder="请选择">
            <el-option v-for="item in  statusOptions" :key="item.key" :label="item.label" :value="item.key">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="删除" prop="isDel">
          <el-select class="filter-item" v-model="marketDynamicInfoParam.isDel" placeholder="请选择">
            <el-option v-for="item in  isDelOptions" :key="item.key" :label="item.label" :value="item.key">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="创建人id" prop="createId">
           <el-input v-model="marketDynamicInfoParam.createId"></el-input>
        </el-form-item>
...
<script>
  import { pageListMarketDynamicInfo, listMarketDynamicInfo, detailMarketDynamicInfo, saveMarketDynamicInfo, updateMarketDynamicInfo, delMarketDynamicInfo } from '@/api/MarketDynamicInfo.js'
  ...
  export default {
    ...
    data() {
      return {
       ...
       statusOptions: [{ label: '审核中', key: 0 }, { label: '通过', key: 1 }, { label: '拒绝', key: 2 }],
        ...
        marketDynamicInfoParam: Object.assign({}, defaultParam),
    ...
    created() {
      this.pageListMarketDynamicInfo()
    },
    methods: {
      ...
      picUploadcoverPic(file) {
        getTempAuthToken().then(res => {
          ...
          axios.post(oss.fileUploadUrl, ossData, {
            ...
            .then(res => {
              this.marketDynamicInfoParam.coverPic = oss.fileUploadUrl + '/' + oss.key
            })
            .catch(error => {
      ...
      // 加载数据
      pageListMarketDynamicInfo() {
        this.listLoading = true
        pageListMarketDynamicInfo(this.listQuery).then(response => {
      ...
      // 显示添加弹窗
      handleAddMarketDynamicInfo() {
        this.dialogStatus = 'create'
        this.dialogVisible = true
        this.marketDynamicInfoParam = Object.assign({}, defaultParam)
        this.$nextTick(() => {
          this.$refs.tinymce_content.setContent('')
        })
      },
      // 显示编辑弹窗
      handleEditMarketDynamicInfo(item) {
        this.dialogStatus = 'update'
        this.marketDynamicInfoParam = item
        this.dialogVisible = true
        this.$nextTick(() => {
      ...
      // 保存或更新
      handleSaveOrUpdate() {
        this.$refs.marketDynamicInfoForm.validate((valid) => {
        	if (valid){
        		// 主键为空 -> 新增
		        if (this.marketDynamicInfoParam.id == null) {
		          saveMarketDynamicInfo(this.marketDynamicInfoParam).then(response => {
		          ...
		          // 有主键 -> 更新
		          updateMarketDynamicInfo(this.marketDynamicInfoParam).then(response => {
		            if (response.code === 600) {
		              this.dialogVisible = false
		              this.pageListMarketDynamicInfo()
		            } else {
      ...
      // 删除
      handleDeleteMarketDynamicInfo(item) {
        this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          delMarketDynamicInfo({ id: item.id }).then(response => {
            if (response.code === 600) {
              this.$message({
                type: 'success',
                message: '删除成功!'
