  ...
  <div class="app-container">
    <div class="filter-container" style="display: flex;justify-content: space-between">
      ...
      <el-form :inline="true" class="demo-form-inline">
        <el-form-item label="app代号">
               <el-input @keyup.enter.native="handleFilter" v-model="listQuery.appCode" placeholder="app代号"></el-input>
        </el-form-item>
      ...
      <div>
        <el-button type="primary" @click="handleAddAppShareInfo">申请</el-button>
      </div>
    </div>

    <el-table :data="list" v-loading="listLoading" border fit highlight-current-row
              style="width: 100%;">
      <el-table-column width="" align="center" label="app代号">
      ...
      <el-table-column align="center" label="操作"  width="" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button type="success" size="mini" @click="handlePassAppShareInfo(scope.row)">通过</el-button>
          <el-button type="danger" size="mini" @click="handleDeleteAppShareInfo(scope.row)">拒绝</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog
      ...
      width="50%">
      <el-form ref="appShareInfoForm" :model="appShareInfoParam" :rules="appShareInfoRules" status-icon label-width="130px">
        <el-form-item label="app代号" prop="appCode">
           <el-input v-model="appShareInfoParam.appCode"></el-input>
        </el-form-item>
        <!-- <el-form-item label="排序" prop="sorts">
           <el-input type="number" v-model="appShareInfoParam.sorts"><i slot="prefix" class="el-input__icon el-icon-goods"></i></el-input>
        </el-form-item>
        <el-form-item label="名称" prop="name">
           <el-input v-model="appShareInfoParam.name"></el-input>
        </el-form-item>
        <el-form-item label="APP主键" prop="appId">
           <el-input v-model="appShareInfoParam.appId"></el-input>
        </el-form-item>
        <el-form-item label="状态" prop="status">
          <el-select class="filter-item" v-model="appShareInfoParam.status" placeholder="请选择">
            <el-option v-for="item in  statusOptions" :key="item.key" :label="item.label" :value="item.key">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="删除" prop="isDel">
          <el-select class="filter-item" v-model="appShareInfoParam.isDel" placeholder="请选择">
            <el-option v-for="item in  isDelOptions" :key="item.key" :label="item.label" :value="item.key">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="创建人id" prop="createId">
           <el-input v-model="appShareInfoParam.createId"></el-input>
        </el-form-item> -->
...
<script>
  import { pageListAppShareInfoAudit, listAppShareInfo, detailAppShareInfo, saveAppShareInfo, updateAppShareInfo, delAppShareInfo,passAppShareInfo } from '@/api/AppShareInfo.js'
  ...
  export default {
    ...
    data() {
      return {
       ...
       statusOptions: [{ label: '审核中', key: 0 }, { label: '通过', key: 1 }, { label: '拒绝', key: 2 }],
	     appShareInfoRules: {
	       appCode: [{ required: true, trigger: 'change', message: 'app代号不能为空' }],
        ...
        appShareInfoParam: Object.assign({}, defaultParam),
      }
    },
    filters: {
      statusFilter(level) {
    ...
    created() {
      this.pageListAppShareInfoAudit()
    },
    methods: {
      ...
      // 加载数据
      pageListAppShareInfoAudit() {
        this.listLoading = true
        pageListAppShareInfoAudit(this.listQuery).then(response => {
      ...
      // 显示添加弹窗
      handleAddAppShareInfo() {
        this.dialogStatus = 'create'
        this.dialogVisible = true
        this.appShareInfoParam = Object.assign({}, defaultParam)
        this.$nextTick(() => {
        })
      },
      // 显示编辑弹窗
      handleEditAppShareInfo(item) {
        this.dialogStatus = 'update'
        this.appShareInfoParam = item
        this.dialogVisible = true
        this.$nextTick(() => {
        })
      },
      // 保存或更新
      handleSaveOrUpdate() {
        this.$refs.appShareInfoForm.validate((valid) => {
        	if (valid){
        		// 主键为空 -> 新增
		        if (this.appShareInfoParam.id == null) {
		          saveAppShareInfo(this.appShareInfoParam).then(response => {
		          ...
		          // 有主键 -> 更新
		          updateAppShareInfo(this.appShareInfoParam).then(response => {
		            if (response.code === 600) {
		              this.dialogVisible = false
		              this.pageListAppShareInfoAudit()
		            } else {
      ...
      // 删除
      handleDeleteAppShareInfo(item) {
        this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          delAppShareInfo({ id: item.id }).then(response => {
            if (response.code === 600) {
              this.$message({
                type: 'success',
                message: '删除成功!'
      ...
      // 通过
      handlePassAppShareInfo(item){
        this.$confirm('确定通过吗?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          passAppShareInfo({ id: item.id }).then(response => {
            if (response.code === 600) {
              this.$message({
                type: 'success',
                message: '操作成功!'
