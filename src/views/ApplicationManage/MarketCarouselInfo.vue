<template>
  <div class="app-container">
    <div class="filter-container" style="display: flex;justify-content: space-between">
      ...
      <el-form :inline="true" class="demo-form-inline">
        <!-- <el-form-item label="轮播图片">
               <el-input @keyup.enter.native="handleFilter" v-model="listQuery.coverPic" placeholder="轮播图片"></el-input>
        </el-form-item>
        <el-form-item label="链接地址">
               <el-input @keyup.enter.native="handleFilter" v-model="listQuery.link" placeholder="链接地址"></el-input>
        </el-form-item> -->
      ...
      <div>
        <el-button type="primary" @click="handleAddMarketCarouselInfo">新增</el-button>
      </div>
    </div>

    <el-table :data="list" v-loading="listLoading" border fit highlight-current-row
              style="width: 100%;">
      <el-table-column width="" align="center" label="轮播图片">
        <template slot-scope="scope">
          <img style="height: 100px" :src="scope.row.coverPic">
        </template>
      </el-table-column>
      <el-table-column width="" align="center" label="链接地址">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" label="操作"  width="" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button type="success" size="mini" @click="handleEditMarketCarouselInfo(scope.row)">编辑</el-button>
          <el-button type="danger" size="mini" @click="handleDeleteMarketCarouselInfo(scope.row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog
      ...
      width="50%">
      <el-form ref="marketCarouselInfoForm" :model="marketCarouselInfoParam" :rules="marketCarouselInfoRules" status-icon label-width="130px">
        <el-form-item label="轮播图片" prop="coverPic">
          <el-upload
            ...
            <img v-if="marketCarouselInfoParam.coverPic" :src="marketCarouselInfoParam.coverPic" class="avatar">
            <i v-else class="el-icon-plus avatar-uploader-icon"></i>
          </el-upload>
        </el-form-item>
        <el-form-item label="链接地址" prop="link">
           <el-input v-model="marketCarouselInfoParam.link"></el-input>
        </el-form-item>
        <el-form-item label="名称" prop="name">
           <el-input v-model="marketCarouselInfoParam.name"></el-input>
        </el-form-item>
        <el-form-item label="内容" prop="content">
          <tinymce v-model="marketCarouselInfoParam.content" :id="tid_content" ref="tinymce_content"/>
        </el-form-item>
        <el-form-item label="排序" prop="sorts">
           <el-input type="number" v-model="marketCarouselInfoParam.sorts"><i slot="prefix" class="el-input__icon el-icon-goods"></i></el-input>
        </el-form-item>
        <!-- <el-form-item label="APP主键" prop="appId">
           <el-input v-model="marketCarouselInfoParam.appId"></el-input>
        </el-form-item> -->
        <el-form-item label="状态" prop="status">
          <el-select class="filter-item" v-model="marketCarouselInfoParam.status" placeholder="请选择">
            <el-option v-for="item in  statusOptions" :key="item.key" :label="item.label" :value="item.key">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="删除" prop="isDel">
          <el-select class="filter-item" v-model="marketCarouselInfoParam.isDel" placeholder="请选择">
            <el-option v-for="item in  isDelOptions" :key="item.key" :label="item.label" :value="item.key">
            </el-option>
          </el-select>
        </el-form-item>
        <!-- <el-form-item label="创建人id" prop="createId">
           <el-input v-model="marketCarouselInfoParam.createId"></el-input>
        </el-form-item> -->
...
<script>
  import { pageListMarketCarouselInfo, listMarketCarouselInfo, detailMarketCarouselInfo, saveMarketCarouselInfo, updateMarketCarouselInfo, delMarketCarouselInfo } from '@/api/MarketCarouselInfo.js'
  ...
  export default {
    ...
    data() {
      return {
       ...
       statusOptions: [{ label: '审核中', key: 0 }, { label: '通过', key: 1 }, { label: '拒绝', key: 2 }],
	     marketCarouselInfoRules: {
	       coverPic: [{ required: true, trigger: 'change', message: '轮播图片不能为空' }],
	       link: [{ required: true, trigger: 'change', message: '链接地址不能为空' }],
        ...
        marketCarouselInfoParam: Object.assign({}, defaultParam),
    ...
    created() {
      this.pageListMarketCarouselInfo()
    },
    methods: {
      ...
      picUploadcoverPic(file) {
        getTempAuthToken().then(res => {
          ...
          axios.post(oss.fileUploadUrl, ossData, {
            ...
            .then(res => {
              this.marketCarouselInfoParam.coverPic = oss.fileUploadUrl + '/' + oss.key
            })
            .catch(error => {
      ...
      // 加载数据
      pageListMarketCarouselInfo() {
        this.listLoading = true
        pageListMarketCarouselInfo(this.listQuery).then(response => {
      ...
      // 显示添加弹窗
      handleAddMarketCarouselInfo() {
        this.dialogStatus = 'create'
        this.dialogVisible = true
        this.marketCarouselInfoParam = Object.assign({}, defaultParam)
        this.$nextTick(() => {
          this.$refs.tinymce_content.setContent('')
        })
      },
      // 显示编辑弹窗
      handleEditMarketCarouselInfo(item) {
        this.dialogStatus = 'update'
        this.marketCarouselInfoParam = item
        this.dialogVisible = true
        this.$nextTick(() => {
      ...
      // 保存或更新
      handleSaveOrUpdate() {
        this.$refs.marketCarouselInfoForm.validate((valid) => {
        	if (valid){
        		// 主键为空 -> 新增
		        if (this.marketCarouselInfoParam.id == null) {
		          saveMarketCarouselInfo(this.marketCarouselInfoParam).then(response => {
		          ...
		          // 有主键 -> 更新
		          updateMarketCarouselInfo(this.marketCarouselInfoParam).then(response => {
		            if (response.code === 600) {
		              this.dialogVisible = false
		              this.pageListMarketCarouselInfo()
		            } else {
      ...
      // 删除
      handleDeleteMarketCarouselInfo(item) {
        this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          delMarketCarouselInfo({ id: item.id }).then(response => {
            if (response.code === 600) {
              this.$message({
                type: 'success',
                message: '删除成功!'
